...
namespace Code.Scripts.UI
{
    public class SegmentedSliderUI : Image
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            for (var i = 0; i < Mathf.Min(segments, (int)(segments * fillAmount) + 1); i++)
            {
                var p = fillAmount * segments - i;
                if (p > 1.0f) p = 1.0f;

                var actualHeight = segments * (1.0f - segmentGap) + (segments - 1) * segmentGap;
                var postScale = new Vector2(1.0f, segments / actualHeight);

                AddQuad(new Vector2(0.0f, i / (float)segments) * postScale,
                    new Vector2(1.0f, (1.0f - segmentGap) * p / segments) * postScale);
            }
        }
